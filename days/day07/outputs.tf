...
output "network_configuration" {
  description = "Network configuration from list type variable"
  value = {
    list_value    = var.network_configuration
    vpc_cidr      = element(var.network_configuration, 0)
    subnet_prefix = element(var.network_configuration, 1)
    cidr_bits     = element(var.network_configuration, 2)
    subnet_full   = "${element(var.network_configuration, 1)}/${element(var.network_configuration, 2)}"
    type          = "list(string)"
  }
}
